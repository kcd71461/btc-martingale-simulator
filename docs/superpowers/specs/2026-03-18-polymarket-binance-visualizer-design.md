...
- **마켓 발견**: Gamma API `GET https://gamma-api.polymarket.com/events?slug=btc-updown-5m-{timestamp}`
  ...
  - 응답에서 `event.markets[n].clobTokenIds` 추출 (Up/Down 각각). `clobTokenIds`는 JSON 문자열이므로 `JSON.parse()` 필요.
  - **주의**: Gamma API는 deprecated 플래그가 있음. 구현 시 HTTP 에러 핸들링 필수. 향후 대체 API로 전환 가능성 있음.
- **실시간 스트리밍**: `wss://ws-subscriptions-clob.polymarket.com/ws/market`
  - 구독: `{ assets_ids: [upTokenId, downTokenId], type: "market", custom_feature_enabled: true }`
  - 수신 이벤트: `book`, `price_change`, `last_trade_price`, `best_bid_ask`, `tick_size_change`
  - Heartbeat: 10초마다 `PING`
- **인증**: 읽기 전용 — 불필요
- **5분 롤오버**: 5분 경계 + 3초 지연 후 Gamma API로 새 마켓 조회 → WS 재구독 (미발견 시 3초 후 재시도, 최대 5회). 5회 실패 시 경고 로그 출력 후 기존 마켓 구독 유지, 다음 5분 경계에서 재시도.

### Binance

- **스트리밍**: `wss://stream.binance.com:9443/ws`로 연결 후 WebSocket API 방식으로 구독:
  ```json
  { "method": "SUBSCRIBE", "params": ["btcusdt@bookTicker", "btcusdt@trade", "btcusdt@depth10@100ms"], "id": 1 }
  ```
  이 방식은 단일 연결에 다중 스트림 구독. 메시지는 플랫 형태로 수신 (combined stream URL의 `{stream, data}` 래핑 없음). 각 메시지의 `e` 필드로 스트림 타입 구분.
- **인증**: 공개 시장 데이터 — 불필요
...
-- 주의: Spot bookTicker에는 E(event time) 필드가 없음. server_ts는 항상 NULL.
-- 시각화에서 timebase=server 선택 시 이 테이블은 local_ts로 폴백.
...
-- 인덱스
CREATE INDEX idx_poly_books_slug_local ON poly_books(market_slug, local_ts);
CREATE INDEX idx_poly_prices_slug_local ON poly_prices(market_slug, local_ts);
...
1. 시작 시 Gamma API로 현재 5분 윈도우 마켓 조회 (`event.markets[n].clobTokenIds` 파싱)
2. WS 연결 및 구독 (`custom_feature_enabled: true` 포함)
3. 수신 루프: 이벤트 타입별 DB INSERT (local_ts 찍고 저장)
   - `book` → poly_books
   - `price_change`, `last_trade_price`, `best_bid_ask` → poly_prices
   - `tick_size_change` → 로그만 출력, 무시
4. 5분 경계 + 3초 후 새 마켓 발견 → WS 재구독 (5회 실패 시 기존 유지)
5. 10초 heartbeat

### Binance Connector

1. WS 연결 후 `SUBSCRIBE` 메시지로 3개 스트림 구독
2. 수신 루프: 스트림별 DB INSERT
3. 연결 끊김 시 3초 후 재연결

### Write Batching (성능)

Binance 스트림은 초당 수백 메시지를 수신. `better-sqlite3`의 동기 INSERT가 이벤트 루프를 블로킹하면 `local_ts` 정확도가 저하됨. 이를 방지하기 위해:

- 인메모리 버퍼에 메시지 축적 (최대 100건 또는 200ms 중 먼저 도달)
- 단일 트랜잭션으로 일괄 INSERT (`db.transaction()`)
- `local_ts`는 버퍼 적재 시점(메시지 수신 즉시)에 찍음, flush 시점이 아님
...
**Server Time 폴백**: Binance bookTicker는 server_ts가 없으므로 `timebase=server` 선택 시 `local_ts`로 폴백. UI에 해당 데이터 소스의 폴백 상태를 표시.
